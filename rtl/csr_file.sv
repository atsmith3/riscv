...
module csr_file
    (input logic clk,
     ...
     // Write data (ignored for RO CSRs)
     input logic csr_we,
     // Write enable (ignored for RO CSRs)
     output logic [31 : 0] csr_rdata,
     // Read data
     output logic csr_valid,
     // 1 if address is valid, 0 for invalid
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      case (csr_addr)
        12'h305:
          mtvec <= csr_wdata; // mtvec is writable
        12'h341:
          mepc <= csr_wdata; // mepc is writable
        12'h342:
          mcause <= csr_wdata; // mcause is writable
        12'h343:
          mtval <= csr_wdata; // mtval is writable
        default: begin
  ...
  always_comb begin
    ...
    case (csr_addr)
      // User-mode CSRs (read-only)
      12'hC00:
        csr_rdata = cycle_counter[31 : 0]; // cycle (lower 32)
      12'hC01:
        csr_rdata = time_counter[31 : 0]; // time (lower 32)
      12'hC02:
        csr_rdata = instret_counter[31 : 0]; // instret (lower 32)
      12'hC80:
        csr_rdata = cycle_counter[63 : 32]; // cycleh (upper 32)
      12'hC81:
        csr_rdata = time_counter[63 : 32]; // timeh (upper 32)
      12'hC82:
        csr_rdata = instret_counter[63 : 32]; // instreth (upper 32)

      // Machine-mode CSRs (read-write)
      12'h305:
        csr_rdata = mtvec; // mtvec
      12'h341:
        csr_rdata = mepc; // mepc
      12'h342:
        csr_rdata = mcause; // mcause
      12'h343:
        csr_rdata = mtval; // mtval
