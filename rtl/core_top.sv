...
module core_top
    (input logic clk,
     ...
     // Byte enables for sub-word memory access
  ...
  byte_lane
      u_byte_lane(
              // Load path: memory -> register file (with byte extraction and
              // sign extension)
  ...
  // During trap, use trap-specific values. During normal CSR ops, use CSR ALU
  // output. These are mutually exclusive (trap signals and csr_write never
  // active together) Refactored to avoid combinational loop warning by
  ...
  assign trap_csr_we =
             load_mepc | load_mcause | load_mtval | (csr_we & csr_write);

  // CSR register file for user-mode counters and machine-mode trap handling
  csr_file u_csr_file(.clk(clk),
                      .rst_n(rst_n),
                      .csr_addr(trap_csr_addr),
                      // Muxed CSR address
                      .csr_wdata(trap_csr_wdata),
                      // Muxed write data
                      .csr_we(trap_csr_we),
                      // Muxed write enable
                      .csr_rdata(csr_rdata),
                      ...
                      .instret_inc(
                          instret_inc) // Increment instruction retired counter
           );

  // CSR ALU for read-modify-write operations
  csr_alu u_csr_alu(.csr_rdata(csr_rdata),
                    // Current CSR value
                    .rs1_or_zimm(csr_operand),
                    // RS1 value or zero-extended immediate
                    .funct3(funct3_csr),
                    // CSR operation from instruction funct3
                    .rs1_is_zero(rs1_is_zero),
                    // Write suppression flag
                    .csr_wdata(csr_wdata),
                    // New CSR value
                    .csr_we(csr_we) // Write enable
          );

  // CSR operand selection: For immediate variants (funct3[2] set), use zimm
  // from rs1 field Otherwise use rs1 register value
  assign csr_operand = funct3_csr[2] ? {27'b0, rs1} : rs1_out;
  assign rs1_is_zero = funct3_csr[2] ? (rs1 == 5'b0) : (rs1 == 5'b0);

  // Increment instret counter when completing an instruction (transitioning to
  // PC_INC or direct to FETCH)
  assign instret_inc = load_pc; // PC is loaded when instruction completes

  // Word-align memory address for sub-word accesses
  // For byte/halfword loads, the memory returns the word containing the
  // byte/halfword The byte_lane module then extracts the correct byte/halfword
  // based on mar_out[1:0]
  ...
  mux8 #(.WIDTH(32))
      u_databus_mux(.sel(databus_mux_sel),
                    .a(pc_out),
                    // DATABUS_PC = 0
                    .b(alu_reg),
                    // DATABUS_ALU = 1 (pipeline register)
                    .c(mdr_out),
                    // DATABUS_MDR = 2
                    .d('b0 /* mar_out */),
                    // DATABUS_MAR = 3
  ...
  mux2 #(.WIDTH(32))
      u_mdr_mux(.sel(mdr_mux_sel),
                .a(load_data_aligned),
                // Load data comes through byte_lane for extraction/extension
                .b(databus),
                .y(mdr_in));

  mux4 #(.WIDTH(32))
      u_rs1_mux(.sel(rs1_mux_sel),
                .a(rs1_out),
                .b(pc_out),
                .c(HALF_WORD_SIZE),
                .d(WORD_SIZE),
  ...
  mux4 #(.WIDTH(32))
      u_rs2_mux(.sel(rs2_mux_sel),
                .a(rs2_out),
                .b('b0),
                .c(imm_reg),
                // Use pipeline register for immediate
                .d(pc_out),
                .y(rs2_mux_out));
